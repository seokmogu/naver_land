...
class CachedTokenCollector:
    ...
    def get_fresh_token(self):
        ...
        try:
            from core.playwright_token_collector import PlaywrightTokenCollector
    ...
    def extract_useful_details(self, detail_data):
        ...
        useful_info = {
            ...
            # 📝 상세 설명
            "상세설명": article_detail.get('detailDescription', '').strip()[:200],
...
def collect_by_cortar_no(cortar_no: str, include_details: bool = True, max_pages: int = 999):
    ...
    try:
        ...
        if total_collected > 0:
            try:
                print(f"💾 DB 직접 저장 시작: {total_collected}개 매물")
                
                # JSON 파일을 다시 읽어서 DB 형식으로 변환
                from collectors.db.json_to_supabase import process_json_file
                db_save_result = process_json_file(filepath, cortar_no)
                if db_save_result.get("success"):
                    print(f"✅ DB 직접 저장 완료: {db_save_result}")
                else:
                    print("⚠️ DB 저장 실패 또는 변환 문제")
            except Exception as e:
                print(f"❌ DB 직접 저장 오류: {e}")
                # DB 저장 실패해도 JSON 파일 수집은 성공으로 간주
        ...
        if result['success']:
            ...
            if db_save_result:
                print(f"💾 DB 저장 통계: {db_save_result}")
            return result
        else:
