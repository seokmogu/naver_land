...
class KakaoAddressConverter:
    def __init__(self, api_key: str = None):
        ...
        """
        # .env 환경변수(KAKAO_REST_API_KEY) 우선 사용
        load_dotenv()  # .env 파일 로드
        self.api_key = api_key or os.getenv('KAKAO_REST_API_KEY') or self._load_api_key_from_config()
        if not self.api_key:
    ...
    def _parse_address_response(self, document: Dict) -> Dict:
        """카카오 API 응답을 파싱하여 주소 정보 추출 (영어 키 사용)"""
