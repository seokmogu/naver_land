    ...
    try:
        with logger.log_collection('강남구', dong_name, cortar_no, f'parallel_collection_{process_name}') as log_context:
            ...
            if collect_result['success']:
                ...
                # 🚨 TEMPORARY: json_to_supabase.py 비활성화 - 잘못된 삭제 로직 방지
                # JSON 파일을 Supabase에 저장
                print(f"⚠️ [{process_name}] Supabase 저장 비활성화됨 (잘못된 삭제 로직 방지): {json_filepath}")
                ...
                supabase_result = {
                    ...
                    'stats': {'new': collected_count, 'updated': 0, 'deleted': 0}
                }
