...
class EnhancedNaverCollector:
    ...
    def _process_article_addition(self, data: Dict, article_no: str = "unknown") -> Dict:
        ...
        try:
            ...
            return {
                ...
                # 🎯 CRITICAL: 백업 면적 정보 (articleSpace가 실패할 경우를 대비)
                'backup_supply_area': data.get('area1'),        # ✅ 실제 API에 존재하는 공급면적
                'backup_exclusive_area': data.get('area2'),     # ✅ 실제 API에 존재하는 전용면적
                'direction': data.get('direction'),             # ✅ 방향 정보
                'floor_info': data.get('floorInfo'),           # ✅ 층 정보 (3/12 형태)
                ...
                # 🎯 DEBUG: 실제 받은 데이터 확인용 (임시)
                '_debug_raw_fields': list(data.keys()) if data else []
            }
        except Exception as e:
    ...
    def _process_article_facility(self, data: Dict, article_no: str = "unknown") -> Dict:
        ...
        try:
            ...
            # ✅ FIXED: 실제 API에서 etcFacilities는 배열로 제공됨!
            facilities_list = data.get('etcFacilities', [])
            facilities_text = ', '.join(facilities_list) if facilities_list and isinstance(facilities_list, list) else str(data.get('etcFacilities', ''))
            facility_code_list = data.get('etcFacilityList', '')
            
            # 배열과 텍스트 모두에서 시설 파싱 (실제 네이버 API 방식)
            facilities = {
            ...
            return {
                ...
                'facility_list': facility_code_list,      # 원본 리스트도 보존
    ...
    def _process_article_space(self, data: Dict, article_no: str = "unknown") -> Dict:
        """articleSpace 섹션 처리 (공간 정보) - 실제 API 필드명으로 완전 수정"""
        try:
            if not data:
            ...
            return {
                ...
                'supply_area': data.get('supplySpace'),         # 공급면적 (실제 필드명)
                'exclusive_area': data.get('exclusiveSpace'),   # 전용면적 (실제 필드명)
    ...
    def _save_property_physical(self, property_id: int, data: Dict):
        ...
        try:
            ...
            # 🔧 면적 데이터 검증 및 백업 활용 (30% 데이터 손실 방지!)
            area_exclusive = safe_float(space_info.get('exclusive_area'))
            area_supply = safe_float(space_info.get('supply_area'))
            
            # 🎯 CRITICAL FIX: articleSpace에서 면적을 못 가져온 경우 articleAddition의 백업 사용
            additional_info = data.get('additional_info', {})
            if area_exclusive is None or area_exclusive <= 0:
                backup_exclusive = safe_float(additional_info.get('backup_exclusive_area'))
                if backup_exclusive and backup_exclusive > 0:
                    area_exclusive = backup_exclusive
                    print(f"🔄 전용면적 백업 사용: {area_exclusive}㎡ (articleAddition.area2)")
                else:
                    area_exclusive = 10.0  # 최소 10㎡ 기본값
                    print(f"⚠️ 전용면적 보정: 10㎡ 기본값 적용")
                    
            if area_supply is None or area_supply <= 0:
                backup_supply = safe_float(additional_info.get('backup_supply_area'))
                if backup_supply and backup_supply > 0:
                    area_supply = backup_supply
                    print(f"🔄 공급면적 백업 사용: {area_supply}㎡ (articleAddition.area1)")
                else:
