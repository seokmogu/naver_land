...
class EnhancedNaverCollector:
    ...
    def print_collection_stats(self):
        ...
        print(f"💰 저장된 세금정보: {self.stats['tax_info_saved']:,}개")  # 새로 추가
        print(f"📊 저장된 시세비교: {self.stats['price_comparisons_saved']:,}개")  # 새로 추가
        print(f"🗺️ 카카오 주소 변환: {self.stats['addresses_enriched']:,}개")  # 카카오 통계!
        print(f"❌ 오류 발생: {self.stats['errors']:,}개")
        
        # 카카오 변환 성공률 표시
        if self.stats['properties_processed'] > 0:
            success_rate = (self.stats['addresses_enriched'] / self.stats['properties_processed']) * 100
